...
public class WheelViewTime extends View {
	...
	/** item文字的颜色 */
	private static final int ITEMS_TEXT_COLOR = 0xF0929292;
...
//	private static final int[] SHADOWS_COLORS = new int[] { 0x00ffffff, 0x00ffffff, 0xffffffff };
	 ...
	 * 3个参数构造函数
	 */
	public WheelViewTime(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 * 2个参数构造函数
	 */
	public WheelViewTime(Context context, AttributeSet attrs) {
		super(context, attrs);
		initData(context);
	}
	...
	private void initResourceIfNecessary() {
		...
		if (centerDrawable == null) {
			centerDrawable = getContext().getResources().getDrawable(R.drawable.time_bubble_line);
		}
...
//		setBackgroundResource(Color.parseColor("#ffffff"));
	...
	private void createLayouts(int widthItems, int widthLabel) {
		if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
			itemsLayout = new StaticLayout(buildText(isScrollingPerformed), itemsPaint, widthItems, widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE
